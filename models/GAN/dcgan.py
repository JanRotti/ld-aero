...
class DCGAN(pl.LightningModule):
    def __init__(self, config, image_key="image", train_imbalance=0.5):
        super().__init__()
        self.config = config
    ...
    def _discriminator_step(self, input):
        ...
        fake_labels = torch.zeros(b).to(self.device)

        real_score = self.criterion(real_score, real_labels)
        fake_score = self.criterion(fake_score, fake_labels)
    ...
    def _generator_step(self, input):
        ...
        gLoss = self.criterion(fake_score, torch.ones(b).to(self.device))
    ...
    def forward(self, x):
        ...
        latent = torch.randn(x.size(0), self.generator.latent_dim).to(self.device)
