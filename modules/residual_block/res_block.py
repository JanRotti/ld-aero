...
class ResBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.initial = nn.Sequential(
            get_norm(norm, self.in_channels, num_groups),
            self.activation,
            nn.Conv2d(self.in_channels, self.out_channels, kernel_size=3, padding=1),
        )
        self.final = nn.Sequential(
            get_norm(norm, self.out_channels, num_groups),
            self.activation,
            nn.Dropout(p=self.dropout),
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3, padding=1),
        )
        self.residual_connection = nn.Conv2d(in_channels, out_channels, 1) if in_channels != out_channels else nn.Identity()
        self.attention = nn.Identity() if not use_attention else AttentionBlock(out_channels, get_norm(norm, out_channels, num_groups))
    ...
    def forward(self, x, time=None, y=None):
        ...
        if self.time_bias is not None:
            if time is None:
                raise ValueError("time conditioning was specified but time_emb is not passed")
            out += self.time_bias(self.activation(time))[:, :, None, None]
        if self.class_bias is not None:
            if y is None:
                raise ValueError("class conditioning was specified but y is not passed")
            out += self.class_bias(y)[:, :, None, None]
        out = self.final(out) + self.residual_connection(x) 
        out = self.attention(out)
        return out
